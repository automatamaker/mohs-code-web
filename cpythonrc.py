...
if not aio.cross.simulator:
    ...
    if (__WASM__ and __EMSCRIPTEN__) or platform.is_browser:
        ...
        # --- MOHS CODE 独自パッチ: パッケージ索引を自ホストに向ける -------
        # aio.pep0723 は PYGPI が未設定だと PKG_BASE_DEFAULT
        # ("https://pygame-web.github.io/archives/repo/") から
        # index-090-cp312.json と repodata.json を取りに行く。これはブート
        # 完走の必須条件で、外部CDNが遅い/館内Wi-Fiが不安定だと青画面のまま
        # 止まる（repodata.json 取得中のハングを再現済み）。
        # 下の DEV MODE が組み立てる PyConfig.pkg_indexes は pep0723 からは
        # 参照されないため、環境変数 PYGPI で指定する必要がある。
        # 同じ2ファイルをバンドルの archives/repo/ に同梱し、そこを指す。
        # これでブートは同一オリジンで完結し、外部通信は不要になる。
        if not os.environ.get("PYGPI", ""):
            try:
                _href = str(platform.window.location.href)
                _base = _href.split("?", 1)[0].split("#", 1)[0].rsplit("/", 1)[0]
                os.environ["PYGPI"] = _base + "/archives/repo/"
                print("807a: PYGPI self-hosted ->", os.environ["PYGPI"])
            except Exception as _e:
                # 失敗したら従来どおり外部CDNへフォールバック（起動を優先）。
                print("807a: PYGPI self-host setup skipped:", _e)
        # -----------------------------------------------------------------
