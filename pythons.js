...
// MOHS CODE 独自パッチ: alert() をやめて console 出力にする。
// alert はモーダルでページ全体を停止させるため、ゲームと無関係なエラーでも
// 致命傷になる。実際 iPhone の Brave が全ページに注入する暗号ウォレットの
// スクリプトが window.ethereum で失敗し、このハンドラがそれを alert して
// ゲームが起動しなかった。バンドル内に ethereum を参照する箇所は無く、
// 完全に第三者由来のエラー。来館者のブラウザは選べないので、こちらの
// 責任範囲外のエラーで停止しないようにしておく。
// 元の挙動が必要なら URL に #debug を付けると alert に戻る。
window.addEventListener("error", function (e) {
   const msg = (e.error && e.error.message) || e.message || String(e)
   console.error("Error occurred:", msg)
   if (location.hash.indexOf("debug") >= 0) alert("Error occurred: " + msg)
   return false;
})
